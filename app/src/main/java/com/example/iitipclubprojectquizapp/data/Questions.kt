package com.example.iitipclubprojectquizapp.data
//list of hard coded questions, API attempt was made but failed, i'll look into it
val Q1 = QuestionAndAns(
    question = "If you could fold a piece of paper in half 50 times, it's; thickness will be 3/4th the distance from the Earth to the Sun.",
    trueAns = false
)

val Q2 = QuestionAndAns(
    question = "The open source program Redis is a relational database server.",
    trueAns = false
)

val Q3 = QuestionAndAns(
    question = "George Clinton, Vice President of the United States (1805-1812), is an ancestor of President Bill Clinton.",
    trueAns = false
)

val Q4 = QuestionAndAns(
    question = "The first U.S. president to visit China was Richard Nixon.",
    trueAns = true
)

val Q5 = QuestionAndAns(
    question = "The Windows ME operating system was released in the year 2000.",
    trueAns = true
)

val Q6 = QuestionAndAns(
    question = "According to Norse mythology, Loki is a mother.",
    trueAns = true
)

val Q7 = QuestionAndAns(
    question = "The name of the popular search engine \"Google\" came from a misspelling of the word \"googol\".",
    trueAns = true
)

val Q8 = QuestionAndAns(
    question = "Seoul is the capital of North Korea.",
    trueAns = false
)

val Q9 = QuestionAndAns(
    question = "The HTML5 standard was published in 2014.",
    trueAns = true
)

val Q10 = QuestionAndAns(
    question = "Metallica collaborated with Rowan Atkinson's Mr Bean on a 1992 comic relief single.",
    trueAns = false
)

val Q11 = QuestionAndAns(
    question = "The British organisation CAMRA stands for The Campaign for Real Ale.",
    trueAns = true
)

val Q12 = QuestionAndAns(
    question = "No Game No Life first aired in 2014.",
    trueAns = true
)

val Q13 = QuestionAndAns(
    question = "The name of the popular programming language Python is derived from the British comedy group Monty Python.",
    trueAns = true
)

val Q14 = QuestionAndAns(
    question = "Theodore Roosevelt Jr. wasn't the only General involved in the initial assault on D-Day.",
    trueAns = false
)

val Q15 = QuestionAndAns(
    question = "The retail disc of Tony Hawk's Pro Skater 5 only comes with the tutorial.",
    trueAns = true
)

val Q16 = QuestionAndAns(
    question = "Abraham Lincoln was the first U.S. President to be born outside the borders of the thirteen original states. ",
    trueAns = true
)

val Q17 = QuestionAndAns(
    question = "The first person convicted of speeding was going 8 mph.",
    trueAns = true
)

val Q18 = QuestionAndAns(
    question = "Slovakia is a member of European Union",
    trueAns = true
)

val Q19 = QuestionAndAns(
    question = "Helen Clark was the 37th Prime Minister of Australia.",
    trueAns = false
)

val Q20 = QuestionAndAns(
    question = "In the original Star Wars trilogy, David Prowse was the actor who physically portrayed Darth Vader.",
    trueAns = true
)

val Q21 = QuestionAndAns(
    question = "Gumbo is a stew that originated in Louisiana.",
    trueAns = true
)

val Q22 = QuestionAndAns(
    question = "DHCP stands for Dynamic Host Configuration Port.",
    trueAns = false
)

val Q23 = QuestionAndAns(
    question = "Vincent van Gogh cut off one of his ears.",
    trueAns = true
)

val Q24 = QuestionAndAns(
    question = "The French word for glass is glace.",
    trueAns = false
)

val Q25 = QuestionAndAns(
    question = "Sugar contains fat.",
    trueAns = false
)

val listOfQuestion = arrayListOf(Q1, Q3, Q4, Q2, Q5, Q6, Q7, Q8, Q9, Q10, Q11, Q12, Q13, Q14, Q15,  Q17,
    Q18, Q19, Q20, Q21, Q22, Q23, Q24, Q25, Q16)